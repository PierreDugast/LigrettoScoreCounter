...
<!-- in:fly="{{ y: 200, duration: 500 }}" out:fly -->
    ...
    <li  in:fly="{{ y: 200, duration: 500 }}" out:fly="{{ y: 200, duration: 500 }}">
            <div class="{win ? 'w-screen max-w-sm md:max-w-lg rounded-lg shadow-lg p-4 mt-10 bg-green-300' : 'w-screen max-w-sm md:max-w-lg rounded-lg shadow-lg p-4 mt-10 bg-white'}">
                ...
                <h3 class="name font-bold text-xl text-gray-900 "><input class="pr-4 pl-4 pt-4 pb-4 rounded-lg shadow-inner bg-gray-100"placeholder="Joueur" on:submit={yell} bind:value={objAttributes.name} ></h3>
                ...
                <p class="text-gray-500 my-1 ">
                    <input class="hist_input shadow-inner" type=text placeholder="" on:submit={updateHistory} bind:value={objAttributes.history} >
                </p>
